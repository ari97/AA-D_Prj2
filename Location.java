...
class Location implements Comparable<Location>{
  ...
  Location nextNeighbor() {
    ...
    if(this.iterationMode == CHANGE_LETTER){
      if(this.indexToChange == (this.word.length()-1)){
        newWord = this.word.substring(0, (this.word.length()-1)) + Character.toString(this.nextLetter);
      } else{
          newWord = this.word.substring(0, this.indexToChange) + Character.toString(this.nextLetter)
                    + this.word.substring((this.indexToChange+1), this.word.length());
        }
    } else if(this.iterationMode == INSERT_LETTER){
        if(this.indexToChange == 0){
          newWord = Character.toString(this.nextLetter) + this.word;
        } else if(this.indexToChange == this.word.length()){
            newWord = this.word + Character.toString(this.nextLetter);
          } else{
              newWord = this.word.substring(0, this.indexToChange)
                        + Character.toString(this.nextLetter)
                        + this.word.substring(this.indexToChange, this.word.length());
            }
    } else{ // DELETE_LETTER mode
        if(this.indexToChange == (this.word.length()-1)){
          newWord = this.word.substring(0, (this.word.length()-1));
        } else{
            newWord = this.word.substring(0, this.indexToChange)
            + this.word.substring((this.indexToChange+1), this.word.length());
        }
        ...
        if(this.nextLetter == 'z'){
          if(this.indexToChange == (this.word.length()-1)){
            ...
            this.nextLetter = 'a';
              ...
              this.iterationMode++;
              this.indexToChange = 0;
            } else{this.indexToChange++; this.nextLetter = 'a';}
        } else{this.nextLetter++;}
      } else if(this.iterationMode ==  DELETE_LETTER){
          if(this.indexToChange == (this.word.length()-1)){
            this.iterationMode++;
          } else{this.indexToChange++;}
        }


    Location newWordLoc = new Location();
    newWordLoc.word = newWord;

    if(newWord.equals(this.word)){ //In this case I just made the same word, so do it again
      newWordLoc = this.nextNeighbor();
    }
    return newWordLoc;
  }
  ...
  boolean isLess(Location loc) {
    // -
    if((this.word.compareTo(loc.word)) < 0){
      return true;
    }
    return false;
  }
